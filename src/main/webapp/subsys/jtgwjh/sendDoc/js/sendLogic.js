...
function updateAipToService(filePath,port,serviceName) {
  ...
  try{   
    var webObj=document.getElementById("HWPostil1");
        ...
        returnValue = webObj.HttpPost("http://"+serviceName+ ":" +port+ "/newtouchHR/subsys/jtgwjh/receiveDoc/saveFile/saveAip.jsp?filePath=" + encodeURIComponent(filePath) + "&fileName=" + encodeURIComponent(attachmentName));
        if("ok" == returnValue){
          return true;
          //alert("上传成功！"); 
